...
import org.seekloud.puppeteer.client.Boot.{blockingDispatcher, captureActor}
import org.slf4j.LoggerFactory
import CaptureActor.{ audioChannels, frameDuration, frameRate}
...
object EncodeActor {
  ...
  def create(logPrefix: String): Behavior[EncodeCommand] = {
    ...
    Behaviors.setup[EncodeCommand] { ctx =>
      Behaviors.receiveMessage[EncodeCommand] {
        case msg: EncodeInit =>
          ...
          val videoEncoder = ctx.spawn(encodeVideoState("videoEncoder|", tsInfo, recorder), "videoEncoder", blockingDispatcher)
          captureActor ! CaptureActor.Encoder(videoEncoder)
